...
(function ($) {
	...
	let DateTextEntry = function (element, options) {
	...
	function pad2(n) {
		return str_right('00' + (n || 0), 2);
	}

	function pad4(n) {
		return str_right('0000' + (n || 0), 4);
	}
	...
	DateTextEntry.prototype = {
		...
		, show_input_tip:         function (input) {
			...
			let x_offset = (input.left() + opt.tooltip_x) + 'px';
		...
		, show_error:             function () {
			...
			if (error_text === '') {
				...
				let x_offset = (this.inner.outerWidth() + opt.errorbox_x) + 'px';
				let y_offset = opt.errorbox_y + 'px';
			...
			if (this.day_value && this.month_value) {
				...
				try {
					...
					if (this.year_value && this.year_value.length === 4) {
						...
						let date_obj = this.get_date();
						let date_str = this.format_date(date_obj);
						this.$element.val(date_str);
						if (this.on_change) {
							this.on_change(date_str, current_input);
						}
					}
				} catch (e) {
					this.set_error(e);
		...
		, validate_days_in_month: function () {
			...
			let msg = opt.E_BAD_DAY_FOR_MONTH;
		...
		, validate_complete_date: function () {
			const opt = this.options;
			const date_obj = this.get_date();
			const date_iso = this.iso_format_date(date_obj);

			let max_date = opt.max_date;
	...
	let DateTextInput = function (options) {
		const input = this;
		let dte = this.dte = options.dte;
		let opt = dte.options;
		...
		this.empty = true;
		this.$input = $('<input type="text" value="" />').addClass('jq-dte-' + this.name).attr('aria-label', this.tip_text + " (" + this.hint_text + ")").focus($.proxy(input, 'focus')).blur($.proxy(input, 'blur')).keydown(function (e) {
			setTimeout(function () {
				input.keydown(e);
			}, 2)
		}).keyup(function (e) {
	...
	$.fn.datetextentry = function (option) {
		...
		return this.each(function () {
			let $this = $(this);
			let data = $this.data('datetextentry');
			if (!data) {
