...
class Search
{
		...
		if ($this->params->get('filter_bedrooms')) {
			...
			foreach ($totalBedrooms as $t) {
				if ($t->id >= $max_bedrooms) {
					$this->data->filterBedrooms = $this->setFilterCount($this->data->filterBedrooms,
					                                                    $max_bedrooms,
					                                                    $t->total);
				}
				else {
	...
	{
		$rates = $this->setForProperty(KrFactory::getListModel('rates')->getRatesForProperty($this->data->baseIds,
		                                                                                     $this->data->arrival,
		                                                                                     $this->data->departure));
	...
	private function validateRegion(): void
	{
		...
		foreach ($this->data->region_id as $id) {
			...
			$this->data->region_name[$id]  = $this->Translations->getText('region', $id);
