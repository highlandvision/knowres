 ...
 * @since 1.0.0
 */
class BedtypesModel extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	/**
	 * Get all bed types as per state param
	 *
	...
	public function getAll(): array
	{
		...
		$query->select($db->qn([
			'id', 'bedicon', 'generic', 'state'
		]));

		$query->select('(' . self::transSQ($db, 'bedtype', 'a.id') . ') AS ' . $db->q('name'))
		      ->from($db->qn('#__knowres_bed_type', 'a'))
		      ->where($db->qn('state') . '=1')
	...
	protected function getListQuery(): QueryInterface
	{
		...
		$query->from($db->qn('#__knowres_bed_type', 'a'));
