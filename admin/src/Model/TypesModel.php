...
class TypesModel extends ListModel
{
	...
	public function getAll(): array
	{
		...
		return $Translations->addTranslationToObject($items, 'type');
	}
	...
	protected function getListQuery(): QueryInterface
	{
		...
		$subQuery->select('sub.text')
		         ...
		         ->where($db->qn('sub.field') . ' = ' . $db->q('abbreviation'))
		...
		$query->from('`#__knowres_type` AS a');
		$query->select('(' . $subQuery->__toString() . ') ' . $db->q('abbreviation'));
		$query->select('(' . $subQuery1->__toString() . ') ' . $db->q('name'));
