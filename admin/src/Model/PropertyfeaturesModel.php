...
class PropertyfeaturesModel extends ListModel
{
	...
	public function getAll(bool $filter = false): array
	{
		...
		$query->from($db->qn('#__knowres_property_feature'))
		      ->where($db->qn('state') . ' = 1');
	...
	//	 * @return array
	//	 */
	//	public function getForProperty(array $features = []): array
	//	{
	//		$db    = $this->getDatabase();
	//		$query = $db->getQuery(true);
	//
	//		$query->select($this->getState('list.select', 'a.id'));
	//		$query->from($db->qn('#__knowres_property_feature', 'a'));
	//		$query->where($db->qn('a.state') . '= 1');
	//		$query->where($db->qn('a.id') . ' IN (' . implode(',', array_map('intval', $features)) . ')');
	...
	protected function getListQuery(): QueryInterface
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('((' . $subQuery->__toString() . ') ' . ' LIKE ' . $search . 'OR' . $db->qn('a.generic')
					. ' LIKE ' . $search . ')');
			}
		}
