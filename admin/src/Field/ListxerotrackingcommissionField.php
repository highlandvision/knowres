 ...
 * @since    3.1.0
 */
class ListxeroatrackingcommissionField extends ListField
{
	/** @var string   The form field type * */
	protected string $type = 'ListxerotrackingcommissionField';

	/**
	 * Get the xero accounts list.
	 *
	 * @since  5.2
	...
	public function getInput(): array
	{
		...
		while (true) {
				...
				if ($d->Status == 'ACTIVE') {
					...
					foreach ($d->Options as $o) {
						if ($o->Status == 'ACTIVE') {
							...
							$xero_tracking[] = HTMLHelper::_('select.option',
			...
			foreach ($types as $r) {
				...
				$html[] = HTMLHelper::_('select.genericlist',
					$xero_tracking,
					"map_commission[$r]",
					implode(' ', $input_options),
					'value',
					'text',
					$default,
					$id);
