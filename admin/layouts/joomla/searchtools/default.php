...
if (isset($data['view']->filterForm) && !empty($data['view']->filterForm))
{
	...
	if ($selectorField)
	{
		$showSelector = $selectorField->getAttribute('filtermode', '') == 'selector';
	...
	if (isset($data['view']->total) && (int) $data['view']->total === 0)
	{
		...
		if (!empty($noResults))
		{
			$noResultsText = KrMethods::plain($noResults);
		}
	}
}
...
foreach ($data['view']->activeFilters as $k => $f)
{
	if ($k != 'departure' && $k != 'payment_date')
	{
		if ($f != '')
		{
			$hideActiveFilters = false;
...
if (!empty($data['view']->related))
{
	if (!$hideActiveFilters && in_array($data['view']->name, $data['view']->related)
		&& count($data['view']->activeFilters) == 1)
	{
		$hideActiveFilters = true;
	}
}
...
$limit = Factory::getApplication()->get('list_limit', 20);

$customOptions = array(
	'filtersHidden'       => isset($data['options']['filtersHidden']) && $data['options']['filtersHidden']
		? $data['options']['filtersHidden'] : $hideActiveFilters,
	'filterButton'        => isset($data['options']['filterButton']) && $data['options']['filterButton']
		? $data['options']['filterButton'] : $showFilterButton,
	'defaultLimit'        => $data['options']['defaultLimit'] ?? $limit,
