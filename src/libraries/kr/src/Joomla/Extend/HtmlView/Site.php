...
class Site extends KrHtmlView
{
    ...
    public static function confirmPathway(Pathway $pathway): Pathway
    {
        ...
        $pathway->addItem(
            Krmethods::plain('COM_KNOWRES_MAKE_A_RESERVATION'),
    ...
    {
        $pathway->addItem(
            KrMethods::plain('COM_KNOWRES_TITLE_DASHBOARD'),
    ...
    public static function propertyRegionPathway(Pathway $pathway, int $region_id, string $region_name): Pathway
    {
        $Itemid = SiteHelper::getItemId('com_knowres', 'properties');
        $link   = KrMethods::route(
            'index.php?option=com_knowres&view=properties' . '&region_id=' . $region_id .
            '&Itemid=' . $Itemid
        );
