...
abstract class ListModel extends \Joomla\CMS\MVC\Model\ListModel
{
     ...
     * @since  3.3.0
     */
    public static function intFilter(DatabaseInterface $db, QueryInterface $query, string $column, mixed $filter,
    ): QueryInterface {
        if (!empty($filter)) {
            if (is_numeric($filter)) {
     ...
     * @since  1.0.0
     */
    public static function jsonFindInSet(DatabaseInterface $db, QueryInterface $query, mixed $filter, string $column,
    ): QueryInterface {
     ...
     * @since   3.3.0
     */
    public static function order(DatabaseInterface $db, QueryInterface $query, string $column, string $direction,
    ): QueryInterface {
        if ($column && $direction) {
     ...
     * @since   3.3.0
     */
    public static function search(DatabaseInterface $db, QueryInterface $query, string $search, string $field,
    ): QueryInterface {
        if (!empty($search)) {
     ...
     * @since  3.3.0
     */
    public static function stringFilter(DatabaseInterface $db, QueryInterface $query, string $column, mixed $filter,
    ): QueryInterface {
        if (!empty($filter)) {
            if (is_string($filter)) {
                $filter = trim($filter);
