...
class PropertyController extends FormController
{
    ...
    public static function geoCodeAddress(string $address): array|bool
    {
        ...
        $url             =
            'https://maps.googleapis.com/maps/api/geocode/json?v=3.55&address=' .
            $address . '&key=' . $key . '&loading=async&callback=initMap';
    ...
    public function calendar(): void
    {
        ...
        $item = KrFactory::getAdminItem('property', $id);
        if (empty($item->id)) {
    ...
    public function checkin(): void
    {
        ...
        if (!$id) {
            ...
            $this->setRedirect(
                KrMethods::route('index.php?option=com_knowres&task=property.dashboard&id=' . $id, false),
            );
    ...
    public function dashboard(): void
    {
        ...
        $view->preview_link = KrMethods::route(KrMethods::getRoot() .
            'index.php?option=com_knowres&view=property&layout=preview&id=' . $id . '&Itemid=' . $Itemid,
        );
        $view->edit_link    = KrMethods::route('index.php?option=com_knowres&task=property.edit&id=' . $id);

        if ($view->item->owner_id) {
            $view->owner = KrFactory::getAdminItem('owner', $view->item->owner_id);
        }
    ...
    public function stats(): void
    {
        ...
        $view->item      = KrFactory::getAdminItem('property', $id);
        $view->settings  = KrFactory::getListModel('propertysettings')->getPropertysettings($id);
    ...
    protected function allowAdd($data = []): bool
    {
        ...
        } elseif ($access_level == 10 && $params->get('property_add')) {
            return true;
        }
    }
    ...
    protected function postSaveHook(BaseDatabaseModel $model, $validData = []): void
    {
        ...
        if (is_countable($fields) && count($fields)) {
            ...
            foreach ($fields as $f) {
                ...
                if ($f->special) {
                    ...
                    $formlabel = $pf->propertyFieldSpecial($f->special, false);
                }

                if (isset($massaged[$formlabel])) {
                    $text = (string)$massaged[$formlabel];
                    $Translations->updateDefault('property', $item->id, $label, $text);
                }
            }
        }
        ...
        if (!(int)$validData['id']) {
            ...
            if ($access_level == 10 || $access_level == 20) {
                /** @var ManagerModel $mm */
                $mm           = KrFactory::getAdminModel('manager');
                $manager      = $mm->getItem($userSession['manager_id']);
            ...
            if ((isset($validData['security_amount']) && $validData['security_amount'] !== $old_security_amount) ||
                ...
                /** @var PropertysettingModel $ps */
                $ps = KrFactory::getAdminModel('propertysetting');
                $ps->updateSetting('security_changes', $validData['id']);
            }

            /** @var ServicequeueModel $sq */
            $sq = KrFactory::getAdminModel('servicequeue');
            $sq::serviceQueueUpdate('updateProperty', (int)$validData['id'], 0, 'ru');
        }
    }
