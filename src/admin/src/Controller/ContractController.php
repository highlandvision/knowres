...
class ContractController extends FormController
{
    ...
    public function agent(): void
    {
        ...
        $model = $this->getModel();
        $form  = $model->getForm();
        if (!$model->validate($form, (array)$contractData)) {
        ...
        try {
            ...
            $agent = KrFactory::getAdminItem('agent', $contractData->agent_id);
            $Hub->setAgent($agent);
        } catch (Exception $e) {
        ...
        if ($Hub->agent->mandatory_extras_charge) {
            $extrasDb =
                KrFactory::getListModel('extras')->getPricingExtras($Hub->getValue('property_id'), true);
    ...
    public function batchxero(): void
    {
        ...
        if (is_countable($cid) && !KrFactory::getListModel('contractpayments')->updateXeroBatch($cid)) {
    ...
    public function delete(): void
    {
        ...
        $id   = $this->validateId();
        $item = KrFactory::getAdminItem('contract', $id);
        if (!$item->id) {
        ...
        $wrapper             = [];
        $wrapper['redirect'] =
            KrMethods::route('index.php?option=com_knowres&view=contracts&success=1', false);
        echo new JsonResponse($wrapper);
        jexit();
    }
    ...
    public function init(): void
    {
        ...
        if (!$edit_id) {
            ...
            if (!$arrival || !$departure) {
                [$arrival, $departure] = $Calendar->getFirstFreeDate();
    ...
    public function manager(): void
    {
        ...
        $contractData                   = $contractSession->updateData($jform);
        $contractData->guests           = (int)$jform['adults'] + (int)$jform['children'];
        $contractData->child_ages       =
            !empty($jform['child_ages']) ? explode(',', $jform['child_ages']) : [];
     ...
     */
    #[NoReturn]
    public function modalbook(): void
    {
     ...
     */
    #[NoReturn]
    public function modalshow(): void
    {
        ...
        if ($type == 'c') {
            ...
            if ($item->black_booking) {
                ...
                $html  = KrMethods::render(
                    'contract.modal.show.block',
                    [
                        'item'         => $item,
                        'notes'        => $notes,
                ...
                if ($item->guestdata_id) {
                    $guestdata = KrFactory::getAdminItem('contractguestdata', $item->guestdata_id);
                }
                ...
                /* @var HighlandVision\Component\Knowres\Administrator\Model\Contractpayment $cp */
                $cp = KrFactory::getAdminModel('contractpayment');
                [$balance, $balance_all] = $cp::setBalances($item, $payments, $fees);

                $html = KrMethods::render('contract.modal.show.reservation', [
                    'item'        => $item,
                    'notes'       => KrFactory::getListModel('contractnotes')->getForContract($item->id),
                    'guest'       => KrFactory::getAdminItem('guest', $item->guest_id),
                    'guestdata'   => $guestdata,
                    'payments'    => $payments,
            ...
            $item = KrFactory::getAdminItem('icalblock', $id);
            if (empty($item->id)) {
                KrMethods::message(KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), 'error');
                echo new JsonResponse(null, KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), true);
                jexit();
    ...
    public function quick(): void
    {
        ...
        $wrapper             = [];
        $wrapper['redirect'] = KrMethods::route(
            'index.php?option=com_knowres&task=contract.show&success=1&id=' . $id, false,
        );
        echo new JsonResponse($wrapper);
        jexit();
    }
     ...
     */
    #[NoReturn]
    public function requestapprove(): void
    {
        $this->checkToken();
        $contract_id = $this->validateId();
        $service_id  = KrMethods::inputInt('service_id');

        $item = KrFactory::getAdminItem('contract', $contract_id);
        if (!$item->id) {
            KrMethods::message(KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), 'error');
            echo new JsonResponse(null, KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), true);
            jexit();
        ...
        if ($service_id > 0) {
            $service = KrFactory::getAdminItem('service', $service_id);
            if (!$service->id) {
                KrMethods::message(KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), 'error');
                echo new JsonResponse(null, KrMethods::plain('COM_KNOWRES_ERROR_TRY_AGAIN_CHECK'), true);
                jexit();
        ...
        /* @var Contractpayments $cps */
        $cps            = KrFactory::getAdminModel('contractpayments');
        $payment        = $cps->getPending($contract_id, $service_id);
     ...
     */
    #[NoReturn]
    public function requestreject(): void
    {
        $this->checkToken();
        ...
        $Hub     = new Hub($contractData);
        $actions = ['requestreject', 'servicequeue', 'emails'];
    ...
    public function resurrect(): void
    {
        ...
        $Hub = new Hub($contractData);
        $Hub->setOriginalData($originalData);
        $actions = ['resurrect', 'servicequeue'];
    ...
    public function trigger(): void
    {
        ...
        $jform         = KrMethods::inputArray('jform');
        $email_trigger = (int)$jform['email_trigger'];
        if ($email_trigger == 0 || $email_trigger == 2) {
            $email = new ContractEmail('MANUALBOOK');
            $email->sendTheEmails($id);
        }

        if ($email_trigger == 1 || $email_trigger == 2) {
        ...
        KrMethods::message(KrMethods::plain('COM_KNOWRES_ACTION_SUCCESS'));
        KrMethods::redirect(
            KrMethods::route('index.php?option=com_knowres&task=contract.show&id=' . $id, false),
        );
    }
    ...
    protected function formatOutputAgent(Hub $Hub): stdClass
    {
        ...
        $output->extra_total      = $Hub->currencyDisplay($Hub->getValue('extra_total'));
        $output->extras           =
            KrMethods::render('contract.summary.extras', ['quote' => $Hub->getData()]);
        $output->adjustmnents     =
            KrMethods::render('contract.summary.adjustments', ['quote' => $Hub->getData()]);
        $output->net_price_system = $Hub->currencyDisplay($Hub->getValue('net_price_system'));
        $output->nights           =
            TickTock::differenceDays($Hub->getValue('arrival'), $Hub->getValue('departure'));
        $output->commission       = $Hub->currencyDisplay($Hub->getValue('commission'));
        $output->room_total       = $Hub->currencyDisplay($Hub->getValue('room_total'));
        $output->contract_total   = $Hub->currencyDisplay($Hub->getValue('contract_total'));
        $output->deposit          = $Hub->currencyDisplay($Hub->getValue('deposit'));
        $output->balance          =
            $Hub->currencyDisplay($Hub->getValue('contract_total') - $Hub->getValue('deposit'));
