...
class ContractguestdataController extends FormController
{
    ...
    protected function postSaveHook(BaseDatabaseModel $model, $validData = []): void
    {
        ...
        if ($contract_id) {
            /** @var $model HighlandVision\Component\Knowres\Administrator\Model\Emailaction */
            $model = KrFactory::getAdminModel('emailaction');
            $model::updateEmailAction($contract_id, 'GUESTARRIVALOWNER');
        }

        if ($this->getTask() == 'save') {
            if (!$contract_id) {
                KrMethods::redirect(
                    KrMethods::route('index.php?option=' . $this->option . '&view=' . $this->view_list
                        . $this->getRedirectToListAppend(), false,
                    ),
                );
            } else {
                KrMethods::redirect(
                    KrMethods::route('index.php?option=' . $this->option . '&task=contract.show&id='
                        . $contract_id, false,
                    ),
                );
            }
