...
class ContractModel extends AdminModel
{
     ...
     * KEEP SPACES ROUND ON & AND AS QUERY FAILS WITHOUT THEM (SEEMS OK IN OTHER FUNCTIONS)
     *
    ...
    public function getItem($pk = null): object|false
    {
        ...
        if ($item) {
            ...
            if (isset($item->property_id) && $item->property_id != 0) {
                /** @var PropertyModel $pm */
                $pm                  = KrFactory::getAdminModel('property');
                $property            = $pm->getItem((int)$item->property_id);
            ...
            if ($item->agent_id > 0) {
                /** @var AgentModel $am */
                $am               = KrFactory::getAdminModel('agent');
                $agent            = $am->getItem((int)$item->agent_id);
            ...
            if ($item->manager_id > 0) {
                ...
                $manager                = $mm->getItem((int)$item->manager_id);
                $item->agency_name      = $manager->agency_name ?? $item->agency_id;
                $item->agency_telephone = $manager->agency_telephone ?? '';
                $item->agency_email     = $manager->agency_email ?? '';
            }

            $item->service_name = '';
            if (!empty($item->service_id)) {
                /** @var ServiceModel $sm */
                $sm      = KrFactory::getAdminModel('service');
                $service = $sm->getItem((int)$item->service_id);
            ...
            if (isset($item->guest_id) && $item->guest_id != 0) {
                ...
                $guest                         = $gm->getItem((int)$item->guest_id);
