...
if (!empty($groupByFieldset))
{
	foreach ($tmpl->getFieldsets() as $fieldset)
	{
		...
		{
			$table_head .= '<span class="icon-info-circle" aria-hidden="true" tabindex="0"></span><div role="tooltip" id="tip-'
				. $field->id . '">' . Text::_($field->description) . '</div>';
		}
...
else
{
	foreach ($tmpl->getGroup('') as $field)
	{
		...
		$table_head .= '<th scope="col">' . strip_tags($field->__get('label'));

		if ($field->__get('description'))
		{
			$table_head .= '<span class="icon-info-circle" aria-hidden="true" tabindex="0"></span><div role="tooltip" id="tip-'
				. $field->__get('id') . '">' . Text::_($field->__get('description')) . '</div>';
		}
...
<div class="subform-repeatable-wrapper subform-table-layout subform-table-sublayout-<?php echo $sublayout; ?>">
	...
	<joomla-field-subform class="subform-repeatable" name="<?php echo $name; ?>"
	                      ...
	                      rows-container="tbody.subform-repeatable-container" minimum="<?php echo $min; ?>"
			...
			<table class="table" id="subfieldList_<?php echo $fieldId; ?>">
				...
				foreach ($forms as $k => $form) :
					echo $this->sublayout($sublayout,
						array('form'    => $form, 'basegroup' => $fieldname, 'group' => $fieldname . $k,
						      'buttons' => $buttons));
				endforeach;
				?>
				</tbody>
		...
		<?php if ($multiple) : ?>
			<template class="subform-repeatable-template-section hidden">
				<?php echo trim($this->sublayout($sublayout,
					array('form'    => $tmpl, 'basegroup' => $fieldname, 'group' => $fieldname . 'X',
					      'buttons' => $buttons))); ?>
			</template>
		<?php endif; ?>
